...
describe("worktree port registry lock", () => {
  it("does not reclaim a stale lock while its fallback ownership probe responds", async () => {
    ...
    // The active owner's heartbeat may refresh the deliberately old mtime
    // before the contender runs. The ownership probe, rather than the mtime,
    // is the invariant under test here.
