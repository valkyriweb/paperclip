...
function canonicalRegularFile(filePath: string, label: string): string {
  ...
  // Accept harmless aliases in parent directories such as macOS's /tmp ->
  // /private/tmp, but reject a symlink at the declared file itself. The latter
  // would let a caller substitute a different source while the former is only
  // an OS pathname normalization detail.
...
export function resolveRegisteredWorktreeSeedSource(
  ...
  if (registeredCwd) {
    ...
    if (lstatSync(resolvedRegisteredCwd).isSymbolicLink()) {
...
export function resolveCanonicalWorktreeSeedSource(input: RegisteredWorktreeSeedSourceInput & {
  ...
  if (canonicalDiagnosticPath !== registered.configPath) {
