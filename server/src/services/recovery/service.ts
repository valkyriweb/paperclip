...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  type ActiveOutputDecisionState = {
    dismissedFalsePositive: boolean;
    quietUntilDecision: Pick<ActiveOutputQuietUntilDecision, "decision" | "snoozedUntil"> | null;
  };

  async function activeOutputDecisionState(
    companyId: string,
    runId: string,
    now = new Date(),
  ): Promise<ActiveOutputDecisionState> {
  ...
  async function buildRunOutputSilence(
    ...
    return runOutputSilenceFromLookups(
      run,
      now,
      quietUntilDecision,
      evaluation,
  ...
  async function latestActiveOutputQuietUntilDecisionsForRuns(
    ...
    runIds: string[],
    now: Date,
  ): Promise<Map<string, ActiveOutputDecisionState>> {
    const byRunId = new Map<string, ActiveOutputDecisionState>();
    ...
    const [rows, dismissedRows] = await Promise.all([
      ...
      db
        .select({ runId: heartbeatRunWatchdogDecisions.runId })
        .from(heartbeatRunWatchdogDecisions)
        .where(
          and(
            eq(heartbeatRunWatchdogDecisions.companyId, companyId),
    ...
    // given runId is that run's latest quiet decision.
    for (const row of rows) {
      if (!byRunId.has(row.runId)) {
        byRunId.set(row.runId, { dismissedFalsePositive: false, quietUntilDecision: row });
      }
    }
    for (const row of dismissedRows) {
      const state = byRunId.get(row.runId) ?? { dismissedFalsePositive: false, quietUntilDecision: null };
      state.dismissedFalsePositive = true;
      byRunId.set(row.runId, state);
    }
    return byRunId;
  }
  ...
  async function buildRunOutputSilenceMap(
    ...
    for (const run of runs) {
      const decisionState = quietUntilDecisions.get(run.id);
      result.set(
        run.id,
        runOutputSilenceFromLookups(
          run,
          now,
          decisionState?.quietUntilDecision ?? null,
          evaluations.get(run.id) ?? null,
          decisionState?.dismissedFalsePositive ?? false,
        ),
      );
    }
    return result;
  ...
  // A board can close an evaluation directly without recording a watchdog decision.
  // Keep that explicit close from causing the scheduler to re-file the same alert.
  ...
  async function inspectSilentActiveRun(input: {
    ...
    // Direct recovery-service inspection is intentionally read-only. The heartbeat
    // scheduler passes issueCreatedAtGte (including null when no worktree cutoff is
    // active) to opt into creating durable evaluation work. This keeps callers that
    // only want to inspect stale runs from mutating the issue graph.
    if (!input.createEvaluation) {
      return existing
        ? { kind: "existing" as const, evaluationIssueId: existing.id }
        : { kind: "skipped" as const };
    }

    // A reviewer may close an evaluation directly on the board. Treat that as a
    // durable dismissal even though no watchdog-decision API row was written.
    ...
    if (closedEvaluation) {
      ...
      if (autoDismissed) return { kind: "skipped" as const };
    }
    ...
    try {
      ...
      if (!isUniqueViolation(error, "issues_active_stale_run_evaluation_uq")) throw error;
  ...
  async function scanSilentActiveRuns(opts?: { now?: Date; companyId?: string; issueCreatedAtGte?: Date | null }) {
    ...
    const createEvaluation = opts !== undefined && Object.prototype.hasOwnProperty.call(opts, "issueCreatedAtGte");
