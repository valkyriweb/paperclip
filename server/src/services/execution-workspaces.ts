...
export function executionWorkspaceService(db: Db, opts: ExecutionWorkspaceServiceOptions = {}) {
  ...
  async function assessDelivery(
    ...
    const { sourceIssueTerminal, subtreeTerminal, cooldownAnchor } = await assessIssueTreeTerminality(workspace);
  ...
  return {
    ...
    sweepTerminalWorkspaces: async (limit = 50) => {
      ...
      for (const workspace of candidates) {
        ...
        const closeReadiness = sharedSession
          ? { git: null, statusInspectionSucceeded: true }
          : await inspectGitCloseReadiness(executionWorkspace);
        if (!closeReadiness.statusInspectionSucceeded) {
          result.skippedUndelivered += 1;
          continue;
        }
        const git = closeReadiness.git;
