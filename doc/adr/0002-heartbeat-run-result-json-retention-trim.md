...
Measured on production (Postgres 17.10), `heartbeat_runs` is **7,455 MB, of which
7,328 MB (98%) is the TOAST heap**. The table itself is small — **16,986 rows in
3,142 main-heap pages** — so essentially all of its size is output blobs hanging
off it. **4,714 runs older than 30 days hold 4,239 MB** of oversized
`result_json`: **58% of the TOAST heap in 28% of the rows**. The largest single
row is 4,123 kB.

Note the shape carefully, because it differs from the run-log problem ADR 0001
solved. This is not a handful of outlier tenants — better than one run in four,
across the whole history, carries an oversized result. Verbose output is the
norm, not the exception, so a rule that only caught outliers would not help.
...
**Forward-only `(created_at, id)` cursor, carried as exact text.** A row can be
oversized for a reason this sweeper cannot fix — bulk in some field that is
neither `stdout` nor `stderr`. Such a row is selected, not modified, and stays a
candidate. With a plain `LIMIT` the sweep would re-select the same page forever;
ordering by `(created_at, id)` and walking past the last row seen guarantees
progress with no marker column and no `->>` predicate. Those rows are reported as
**`residue`** (`examined - trimmed`); a persistently non-zero residue means
something else in `result_json` has grown large and needs its own look.

The cursor timestamp is selected as `created_at::text` and bound straight back
as an opaque token. **It must never pass through a JavaScript `Date`.**
`created_at` is `defaultNow()`, so stored values carry microseconds — 1,997 of
2,000 sampled production rows have a non-zero sub-millisecond component. A `Date`
holds only milliseconds and `toISOString()` truncates rather than rounds, so a
round-tripped cursor always lands slightly *before* the row it came from. Row
comparison settles on the first element, so `(created_at, id) > (truncated, id)`
is true for that very row: it returns at the head of every subsequent page, the
loop never sees an empty page, and the sweep runs to `itemLimit` — 2,000 full
sequential scans — while trimming nothing. That is not an edge case but the
design's own steady state, since once the backlog clears every remaining
candidate is residue. It also poisons `residue` itself, which would report
`itemLimit` rather than the true count and make the signal below meaningless.
A real-Postgres test seeds rows through the schema default and asserts they carry
microseconds before paging them, so the fixture cannot quietly stop testing this.
...
**The stamp has to stay honest, because it is all that is left.** Two details
follow from that. `originalSizeBytes` records the *logical* size
(`octet_length(result_json::text)`), not `pg_column_size` — the API projection
stamps that key from `pg_column_size`, but its value is ephemeral and the row
behind it is intact, whereas this one is the durable record of what was
destroyed, and the compressed stored size can read an order of magnitude smaller
than the text that was actually there. And `stdoutTruncated` / `stderrTruncated`
are OR'd with any value already on the document, so a producer that had already
marked the output truncated cannot be downgraded to `false` by this sweeper —
writing "this output is complete" over something that was already cut short is
the one lie this metadata must never tell.
...
invariant: worst-case trimmed size must stay under the gate, or every trimmed row
would remain a permanent candidate. Note the units differ — the cap counts
*characters* (`left`/`length`) while the gate counts *bytes* (`pg_column_size`) —
so the worst case assumes every kept character is a 4-byte one:
`2 x 4096 x 4 + 512 = 33,280` bytes against a 65,536 byte gate, a 1.97x margin.
That is pessimistic twice over, since the gate measures the compressed stored
size while the calculation counts raw bytes.
...
- **The selector is a sequential scan.** All thirteen indexes on `heartbeat_runs`
  ...
  Scan + Sort, total cost 3670.86, est 3414 rows, over a 3,142-page / 16,986-row
  main heap — about 0.75 s. Acceptable for a daily job: with an exact cursor the
  steady state is one scan per batch and residue rows are walked past rather than
  re-paged. No index was added because `pg_column_size` is not indexable and
  `CREATE INDEX CONCURRENTLY` cannot run inside a migration transaction.
- **The first sweep rewrites the whole backlog in one burst.** Each batch is its
  own autocommit transaction, and there is no pause between them. At the measured
  averages the 4,239 MB backlog is ~24 back-to-back batches — several GB of TOAST
  chunk deletions, all WAL-logged, with a full-page image per first-touched page
  under `full_page_writes`, plus that much dead tuple for autovacuum to chase.
  **Do the first pass throttled**: set
  `PAPERCLIP_RUN_RESULT_RETENTION_ITEM_LIMIT=3` (600 rows per sweep) and raise it
  once WAL and replication lag look boring. Steady state after that is a handful
  of rows a day and this does not arise again.
- **Overlapping sweeps are suppressed within a process, not across processes.** A
  tick that fires while the previous sweep is still walking returns
  `skipped: 'already_running'`. Two server replicas sweeping at once is still
  correct — the second blocks on the row lock, then re-evaluates the `WHERE`
  against the committed version under `READ COMMITTED` and updates nothing — but
  it duplicates the sequential scans and the lock waits. Cheap enough to accept;
  if that changes, an advisory lock is the fix.
