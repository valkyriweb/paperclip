...
describe("IssueProperties", () => {
  ...
  it("renders scheduled, retrying, due, overdue, cleared, and empty monitor row states", async () => {
    ...
    // The absolute time follows the machine locale/timezone, while the
    // countdown and attempt suffix are timezone-independent.
    expect(monitorRowText()).toContain(`${formatMonitorAbsolute(at(132), {}, NOW)} · Attempt 1`);
    ...
    await flush();
    expect(monitorRowText()).toContain("In 2h 12m");
    expect(monitorRowText()).toContain(formatMonitorAbsolute(at(132), {}, NOW));
    ...
    expect(monitorRowText()).toContain(`${formatMonitorAbsolute(at(-18), {}, NOW)} · fires on next tick`);
