...
export function invalidateInboxIssueQueries(queryClient: QueryClient, companyId: string) {
  return Promise.all([
    ...
    // Fork: an issue can be both "touched by me" and blocked at the same
    // time, so archive/read-state changes must also refresh the Blocked tab's
    // own query — otherwise it keeps showing stale pre-action state.
